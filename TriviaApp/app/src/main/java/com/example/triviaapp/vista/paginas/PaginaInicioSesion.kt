...
import com.example.triviaapp.viewModels.vm.PaginaInicioSesionViewModel

/**
 * Pagina para entrar en tu cuenta ya creada
 */
@Composable
fun PaginaLogin(
    paginaLoginUi : PaginaInicioSesionViewModel = viewModel(),
    onItemClick: () -> Unit,
    onSignUpClick:()->Unit,
) {
            ...
            Column(verticalArrangement = Arrangement.spacedBy(space = 35.dp)) {
                ComponenteTextField(
                    DatosTextField(
                        ...
                        txtContenido = uiState.stringNombre,
                        accion = { paginaLoginUi.setNombre(it)
                        }
                    )
                )
